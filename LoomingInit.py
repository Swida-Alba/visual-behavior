...
import time
import numpy as np
import LoomingFunc as loom
from dataclasses import dataclass
from datetime import datetime, timedelta

@dataclass
class LoomingParameters:
    ...
    protocol_save_path: str = ''
    '''path of the protocol file to save the current experiment sessions'''
    
    frame_rate: int = 240
    '''frame rate of the video, as part of the path to the looming files'''
    
    dpi: int = 120
    '''dpi of the video, as part of the path to the looming files'''
    
    loom_retention: int = 1000
    ...
    '''retention time (ms) of the LEDs in the looming-LED timer mode'''
    
    backgroud_color: str = 'w'
    '''background color of looms, as part of the path to the looming files'''
    
    shape: str = 'full'
    '''end shape of the looming object, as part of the path to the looming files'''
    
    background_img = np.full((500,500,3),255,dtype=np.uint8)
    '''background image path of the looms'''
    
    t_today: str = datetime.now().strftime('%Y%m%d')
    '''date running the experiment, as part of the file path to save the current log'''
    
    looms_dir: str = os.path.join(script_path, '_'.join([backgroud_color,shape,str(frame_rate)+'Hz',str(dpi)+'dpi']))
    '''path to the looming files'''
    
    looms_files = list()
    '''list of looming files'''
    
    loom_dict = dict()
    '''dictionary of looming files, with the key being the looming r/v value'''
    
    frms = list()
    '''frames of the looming video, read from the video files'''
    
    read_fps: int = 0
    '''frame rate of the video, read from the video files'''
    
    save_path: str = ''
    '''path to save the current log'''
    ...
    loom_times: int = 0
    '''number of times to play looming video'''
    ...
    def __post_init__(self):
        ...
        self.protocol_save_path = os.path.join(self.protocol_dir, 'autosaved_'+datetime.now().strftime('%Y%m%d_%H%M%S')+'.txt')
        # initializing palyer background
        self.looms_dir: str = os.path.join(self.script_path, '_'.join([self.backgroud_color,self.shape,str(self.frame_rate)+'Hz',str(self.dpi)+'dpi']))
        self.background_img = cv2.imread(os.path.join(self.script_path, 'bg_'+self.backgroud_color+'.png'))
        self.window_bg = loom.initialize_window(self.background_img, window=self.window_bg)
        
        # initializing serial port
        if not self.ser:
            self.ser = loom.SetUpSerialPort(board_type=self.board_type)
        ...
        # loading stimulus videos
        if not self.looms_dir:
            self.looms_dir = os.path.join(self.script_path, '_'.join([self.backgroud_color,self.shape,str(self.frame_rate)+'Hz',str(self.dpi)+'dpi']))
        self.looms_files = [file for file in os.listdir(self.looms_dir) if file.endswith(self.video_format)]
        self.loom_dict = dict()
        for i in range(len(self.looms_files)):
            # the video name should be rv_XXX.format, the XXX is the r/v value
            self.loom_dict.update({int(self.looms_files[i].split('.')[0][3:]): loom.read_video(os.path.join(self.looms_dir, self.looms_files[i]))})
            print(f'\rLoading looming files from: {self.looms_dir} ...loaded {i+1}/{len(self.looms_files)}', end='')
        print()
        self.valid_r2v = sorted(list(self.loom_dict.keys()))
        print(f'Available r/v values: {self.valid_r2v}')
        self.frms, self.read_fps = self.loom_dict[self.r2v]
        self.window = loom.initialize_window(self.frms[0], window=self.window) # initialize the video player window
        print(f'Current r/v = {self.r2v} ms')
        
        # checking the existed log files and building new log file if required
        self.existed_logfiles = loom.get_existed_files(self.save_path,prefix = self.t_today)
        print('Existed log files:')
        for file in self.existed_logfiles:
            print('\t'+file)
        print()
        if self.log_name:
    ...
    def update_mutable_attr(self,key_input):
        ...
        if not hasattr(self,attr):
            ...
            return self.reset_r2v(val)
        elif attr not in self.mutable_attrs:
            print(f'Attribute {attr} is not immediately mutable!')
            print(f'Immediately mutable attributes: {self.mutable_attrs}')
            return
        elif attr in ['LED_retention','loom_retention','loomLED_timer','well_times','pulse_width']:
            if val.isnumeric():
                val = int(val)
            else:
    ...
    def say_u(self):
        ...
        uindow = loom.initialize_window(img_to_show=umage,window='umage_uindow',window_size=(400,600),window_pos=(800,150),full_screen=False,always_on_top=True)
    ...
    def terminate(self):
        ...
        loom.LED_switch(self.LED_state,self.ser,self.log_file,turn_on=False)
        cv2.destroyAllWindows()
        if self.ser: self.ser.close()
        print('Sessions terminated.')
        if os.path.exists(self.protocol_save_path): print(f'Current protocol was saved as: {self.protocol_save_path}')
        return 0
    
    def deliver_looming(self):
        print(f'r/v = {self.r2v} ms')
        real_fps, interval, t_play, t_end = loom.play_video(self.frms, self.read_fps, self.window, retention_time=self.loom_retention)
        loom.write_video_log(self.log_file, real_fps, self.read_fps, self.r2v, t_play, t_end, self.LED_state, interval)
        self.window = loom.initialize_window(self.frms[0])
        return 0
    
    def deliver_looming_series(self, loop_times = None):
        if loop_times is not None:
            self.loom_times = loop_times
            for i in range(self.loom_times):
                self.deliver_looming()
        else:
            loop_input = input(f'Please input the loop times (r/v = {self.r2v} ms): ')
            if loop_input.isnumeric():
                self.loom_times = int(loop_input)
                print(f'Looming series: {self.loom_times} times')
                with open(self.log_file,'a') as log:
                    log.write(f'Looming series: {self.loom_times} times\n')
                    log.write('\n')
                for i in range(self.loom_times):
                    self.deliver_looming()
            else:
    ...
    def reset_r2v(self, r2v_new = None):
        if r2v_new is None:
            r2v_new = input(f'Please input the new r/v (current r/v = {self.r2v} ms): ')
        if r2v_new.isnumeric():
            r2v_new = int(r2v_new)
            if r2v_new in self.valid_r2v:
                self.r2v = r2v_new
                self.frms, self.read_fps = self.loom_dict[self.r2v]
                print(f'r/v is reset to {self.r2v} ms.')
                return 0
            else:
                print(f'Input r/v not available, please input a valid r/v in {self.valid_r2v}.')
                return
        else:
            print(f'Invalid input, please input an integer in {self.valid_r2v}.')
            return
    
    def LED_controller(self, key_input = 'r'):
        '''control the LED; switch on/off or set the ON time'''
        if key_input == 'r': # turn on/off the LED
            self.LED_state = loom.LED_switch(self.LED_state,self.ser,self.log_file)
        elif key_input[1:].replace('.','').isnumeric(): # set the LED ON time if the LED is off
            LED_t = float(key_input[1:])
            loom.LED_timer(self.LED_state,self.ser,self.log_file,timer=LED_t)
        else:
    ...
    def LED_pulse_controller(self):
        ...
        if loom.LED_check(self.LED_state, self.ser) != 0: return
        if self.update_pulse:
            ...
            if d_input.replace('.','').isnumeric() and f_input.replace('.','').isnumeric() and pw_input.isnumeric():
                ...
                return
        loom.LED_pulse(self.LED_state,self.ser,self.log_file,duration=self.pulse_span,frequency=self.pulse_frequency,pulse_width=self.pulse_width)
        return 0
    
    def loomLED_coordination(self):
        '''
        Coordinated and timed LED and looming stimuli
        not user-friendly enough, need to be improved
        '''
        if loom.LED_check(self.LED_state, self.ser) != 0: return
        else:
            if self.update_timer:
                timer_input = input(f'\nPlease input the timer (s) or press <Enter> to use the latest timer ({self.loomLED_timer} ms)\npositive for LED->looming, negative for looming->LED: ')
                if timer_input != '':
                    if timer_input.replace('-','').replace('.','').isnumeric():
                        self.loomLED_timer = int(float(timer_input)*1000) # update timer and convert to ms
                    else:
                        print('Invalid input, please input a number.')
                        return
            loom_time = len(self.frms) / self.read_fps + self.loom_retention/1000
            print_timer = (self.LED_retention - self.loomLED_timer)/1000 - loom_time
            loom.LED_timer(self.LED_state, self.ser, self.log_file, timer=self.LED_retention/1000, delay=-self.loomLED_timer/1000, wait_for_feedback=False) # delay only works when greater than 0
            loom.time_delay(self.loomLED_timer/1000,prefix='Waiting for looming:',suffix='s') # only works when the timer is positive
            self.deliver_looming()
            if print_timer > 0: loom.time_delay(print_timer,prefix='Waiting for LED END:',suffix='s')
        return 0
    ...
    # def CoLoomLED(self):
        '''
        Coordinated and timed LED and looming stimuli
        not recommended to use due to delay in serial communication
        significant frame lost !!!!!!!!!!!
        currently not used
        '''
        if loom.LED_check(self.LED_state, self.ser) != 0: return
        if self.update_timer:
            timer_input = input(f'\nPlease input the timer (s) or press <Enter> to use the latest timer ({self.loomLED_timer} ms)\npositive for LED->looming, negative for looming->LED: ')
            if timer_input != '':
                if timer_input.replace('-','').isnumeric():
                    self.loomLED_timer = int(float(timer_input)*1000) # update timer and convert to ms
                else:
                    print('Invalid input, please input a number.')
                    return
        
        t_video = len(self.frms) / self.read_fps # in seconds
        t_video_total = t_video + self.loom_retention/1000 # in seconds
        t_LED_ON = self.LED_retention/1000
        if self.loomLED_timer >= 0: # red LEDs will be turned on before (or at the same time as) looming
            t_total = max(self.loomLED_timer/1000 + t_video_total, t_LED_ON)
            cmd_t = 'r' + str(int(t_LED_ON*1000)) + '\n'
            t_on = datetime.now()
            t_video_on = self.loomLED_timer/1000
        else: # red LEDs will be turned on after looming
            t_total = max(-self.loomLED_timer/1000 + t_LED_ON, t_video_total)
            cmd_t = 'r' + str(int(t_LED_ON*1000)) + 'd' + str(int(-self.loomLED_timer)) + '\n'
            t_on = datetime.now() + timedelta(seconds=-self.loomLED_timer/1000)
            t_video_on = 0
        self.ser.write(cmd_t.encode('utf-8'))
        t_off = t_on + timedelta(seconds=t_LED_ON)
        t_play = t_on + timedelta(seconds=t_video_on)
        t_end = t_on + timedelta(seconds=t_video_total)
        t_on = t_on.strftime('%Y%m%d%H%M%S%f')[:-3]
        t_off = t_off.strftime('%Y%m%d%H%M%S%f')[:-3]
        
        t_video_off = t_video_on + t_video_total
        t_timeout = t_total + 5
        t_start = time.time()
        t_elapsed = 0
        feedback_ON = False
        ...
        frmN = 0
        t_seq = []
        while t_elapsed < t_total or (not feedback_ON) or (not feedback_OFF):
            t_current = time.time()
            t_elapsed = t_current - t_start
            # timeout
            if t_elapsed > t_timeout: 
                print('\n\033[33mArduino Timeout reached, please check the connection.\033[0m\n')
                return
            ...
            if self.ser.inWaiting() > 0: 
                ...
                print(fb)
            # video time
            if t_elapsed >= t_video_on and t_elapsed <= t_video_on + t_video: # play video
                curr_frame = int((t_elapsed - t_video_on) * self.read_fps)
                if curr_frame > last_frame:
                    cv2.imshow(self.window, self.frms[curr_frame])
                    cv2.waitKey(1)
                    frmN += 1
                    t_seq.append(t_elapsed*1000)
                    frmN_lost += curr_frame - last_frame - 1
                    last_frame = curr_frame
            elif t_elapsed > t_video_on + t_video and t_elapsed <= t_video_off: # retain last frame
                cv2.imshow(self.window, self.frms[-1])
                cv2.waitKey(1)
            elif t_elapsed > t_video_off and not isWindowInitialized: # turn off video window
                cv2.imshow(self.window, self.frms[0])
                cv2.waitKey(1)
                isWindowInitialized = True
        print(f'lost frames: {frmN_lost}')
        interval = np.diff(t_seq)
        playFPS = frmN / t_video
        with open(self.log_file,'a') as log:
    ...
    def write_protocols(self,key_input):
        ...
        if hasattr(self,'t_last_stim'):
            ...
            with open(self.protocol_save_path,'a') as f:
                f.write(cmd_write + ' '*(40-len(cmd_write)) + ' # ' + cmt_write + '\n\n')
        else:
            self.t_last_stim = time.time()
        if key_input == '':
            cmd_write = 'Loom' # command to write
            cmt_write = 'single looming stimulus' # comment to write
        elif key_input == 'v':
            cmd_write = 'Loom ' + str(self.loom_times)
            cmt_write = 'looming for ' + str(self.loom_times) + ' times'
        elif key_input == 'rv':
            cmd_write = 'r/v ' + str(self.r2v)
            cmt_write = f'set r/v to {self.r2v}'
        elif key_input[0] == 'r':
            if key_input == 'r':
                cmd_write = 'LED'
                if self.LED_state:
            ...
            cmd_write = 'LEDandLoom ' + str((self.loomLED_timer)/1000)
            cmt_write = f'coordinated looming and LED with timer = {self.loomLED_timer} ms'
            ...
            if protocol_input in range(1,len(protocol_files)+1):
                ...
                for line in protocol:
                    ...
                    if cmd[0].lower() == 'isi': # inter-stimulus interval
                        ...
                        print()
                    elif cmd[0].lower() == 'loom':
                        if cmdlen == 1:
                            self.deliver_looming()
                        else:
                            self.deliver_looming_series(int(cmd[1]))
                    ...
                    elif cmd[0].lower() == 'ledandloom':
                        if cmdlen == 1:
                            self.loomLED_coordination()
                        else:
                            loomLED_backups = [self.loomLED_timer, self.update_timer]
                            self.update_timer = False
                            self.loomLED_timer = int(float(cmd[1])*1000)
                            self.loomLED_coordination()
                            self.loomLED_timer, self.update_timer = loomLED_backups
                    elif cmd[0].lower() == 'r/v':
                        self.reset_r2v(r2v_new=cmd[1])
    ...
    def show_help(self,key_input='h'):
        ...
        help_dict = {
            ...
            "rv": [
                "update the r/v value, r/v value should be included in the loaded videos",
                "r/v value well be asked after pressing 'rv', usually in [20, 40, 80, 160, 320]",
            ],
            ...
            "set": [
                ...
                "e.g. 'set:loomLED_timer = 5000' for updating loomLED_timer to 5 s, and set update_timer to False",
                "you can also set the r/v value by this method e.g. 'set:r2v = 40'",
                "Don't forget the colon ':' after 'set'\n",
                "\033[34mparameters (attributes)\033[0m that can be set: ",
                "loomLED_timer: int, unit: ms",
                "LED_retention: int, unit: ms",
                "loom_retention: int, unit: ms",
                "well_times: int",
                ...
                "r2v: int, should be in the candidate list, unit: ms",
            ],
            ...
            "<Enter>": 
                "Press <Enter> key directly to deliver looming stimulus",
            
            "v": 
                "deliver looming stimulus for multiple times, times will be asked after entering 'v' mode",
            ...
            "t": [
                "deliver looming stimulus after/before a LED timer, the timer time well be asked after pressing 't'",
                "e.g. later input 5 for looming being set to 5 s after LED; -2.5 for LED being set to 2.5 s after looming",
                "e.g. 0 means looming and LED are delivered simultaneously",
                ...
                "but you can always change the timer time by input 'update:loomLED_timer = number' whose unit is ms",
            ],
    ...
    def start_journey(self):
        '''start the interactive user interface, journey to the looming world'''
        while True:
            execute_state = None
            ...
            if key_input == 'q': # quit
                ...
                execute_state = self.deliver_looming()
            elif key_input == 'v': # play video series
                execute_state = self.deliver_looming_series()
            elif key_input == 'rv': # reset r/v value
                execute_state = self.reset_r2v()
            ...
            elif key_input == 't': # timer for coordinating the LED and looming
                execute_state = self.loomLED_coordination()
                # execute_state = self.CoLoomLED()
                ...
                self.run_protocols()
            elif key_input.startswith('set:'): # update the parameters of looming, e.g. self.r2v = 40, or show the attributes, e.g. self.r2v
