...
import moviepy.editor # Magic happends here, but I quit figuring out why. The moviepy.editor improves the performance of the video playing dramatically.
import numpy as np
...
def play_video(frame_list, fps=200, window='image', retention_time = 1000):
...
def write_video_log(log_path, real_fps, read_fps, r2v, t_play, t_end, LED_state, interval, frmN_lost=0):
    print('Elapsed: {:.2f} s'.format(np.sum(interval)/1000))
    print('Real r/v = {:.2f} ms at {:.2f} fps'.format(read_fps/real_fps*r2v, real_fps))
    ...
    with open(log_path,'a') as log: # save the log file
        ...
        log.write('Real r/v = {:.2f} ms at {:.2f} fps\n'.format(read_fps/real_fps*r2v, real_fps))
        if frmN_lost > 0:
            log.write('Lost frames: {}\n'.format(frmN_lost))
        log.write('LED state: {}\n'.format(LED_state))
        log.write('\n')
    return
...
def LED_timer(LED_state,ser,log_path,timer,delay=0,wait_for_feedback=True):
    ...
    if delay <= 0:
        delay = 0
        print(f'LED timer: {timer:.3f} s')
    elif delay > 0:
        print(f'Delayed LED timer: {timer:.3f} s after {delay:.3f} s delay')
    ...
    if wait_for_feedback:
        ...
        while True:
            ...
            if LED_state and timer > 1: # if timer is longer than 1 second
                print('\rLED left: {:.2f} s   '.format(timer-(time.time()-t_start)),end='')
            if ser.inWaiting() > 0:
...
def LED_pulse(LED_state,ser,log_path,duration,frequency,pulse_width):
    ...
    while True:
        ...
        if LED_state and duration > 1: # if timer is longer than 1 second
            print('\rLED left: {:.2f} s   '.format(duration-(time.time()-t_start)),end='')
        if ser.inWaiting() > 0:
            fb = ser.readline().decode()[:-1]
            if fb == 'Pulsing ON':
                t_on_fb = datetime.now().strftime('%Y%m%d%H%M%S%f')[:-3]
