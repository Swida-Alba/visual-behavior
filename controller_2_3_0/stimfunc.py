...
def continuous_pulse_switch(pulse_state, ser, log_path, turn_on=None, frequency=1.0, pulse_width=50):
    """
    Control the continuous LED pulsing (turn on/off)
    
    Parameters:
        pulse_state (bool): Current pulse state (True=on, False=off)
        ser: Serial port object
        log_path (str): Path to log file
        turn_on (bool, optional): Force on/off state. Defaults to None (toggle).
        frequency (float): Pulse frequency in Hz
        pulse_width (int): Pulse width in ms
        
    Returns:
        bool: New pulse state
    """
    if ser == '':
        print('\033[33mSerial port not available. Cannot control continuous pulse.\033[0m')
        return pulse_state
    
    # Format frequency with 3 decimal places precision (x1000)
    freq_val = int(frequency * 1000)
    
    # Determine command based on current state and parameters
    if turn_on is None:
        # Toggle mode
        if pulse_state:
            cmd = "pulse:off\n"
            print(f'Turning continuous pulse OFF')
        else:
            cmd = f"pulse:f{freq_val}w{pulse_width}\n"
            print(f'Setting pulse parameters (freq: {frequency} Hz, width: {pulse_width} ms)')
    elif turn_on:
        cmd = f"pulse:f{freq_val}w{pulse_width}\n"
        print(f'Turning continuous pulse ON (freq: {frequency} Hz, width: {pulse_width} ms)')
    else:
        cmd = "pulse:off\n"
        print('Turning continuous pulse OFF')
    
    # Log action time
    t_pulse = datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]
    ser.write(cmd.encode('utf-8'))
    
    # Wait for feedback from Arduino - accept multiple possible responses
    t_timeout = 5  # timeout in seconds
    t_arduino = time.time()
    feedback_received = False
    
    # Keep track of which message we received
    received_message = ""
    
    while not feedback_received and (time.time() - t_arduino < t_timeout):
        if ser.in_waiting > 0:
            response = ser.readline().decode('utf-8').strip()
            print(response)
            
            # Accept any of these messages as valid feedback
            valid_on_messages = ["Continuous Pulse ON", "Pulsing ON"]
            valid_off_messages = ["Continuous Pulse OFF", "Pulsing OFF"]
            valid_update_messages = ["Pulse parameters updated"]
            
            if any(msg in response for msg in valid_on_messages):
                feedback_received = True
                received_message = "ON"
            elif any(msg in response for msg in valid_off_messages):
                feedback_received = True
                received_message = "OFF"
            elif any(msg in response for msg in valid_update_messages):
                feedback_received = True
                received_message = "UPDATED"
    
    if not feedback_received:
        print('\033[33mTimeout waiting for Arduino response\033[0m')
    else:
        # Update pulse state based on the message received
        if received_message == "ON":
            pulse_state = True
        elif received_message == "OFF":
            pulse_state = False
        # For "UPDATED", don't change the state
        
        # Log the action
        with open(log_path, 'a') as log:
            if received_message == "UPDATED":
                log.write('{}\tPulse parameters\tFreq:{:.3f}Hz Width:{}ms\n'.format(
                    t_pulse, frequency, pulse_width
                ))
            else:
                log.write('{}\t{}\tContinuous Pulse {}\n'.format(
                    t_pulse, received_message, received_message
                ))
...
def set_pin_value(ser, log_path, pin_index, value):
    """Set the value of a specific pin without waiting for feedback
    
    Parameters:
        ser: Serial port object
        log_path (str): Path to log file
        pin_index (int): Index of the pin
        value (int or str): Value to set (0/1/LOW/HIGH for digital, 0-255 for PWM)
        
    Returns:
        int: 0 if command sent successfully, 1 otherwise
    """
    if ser == '':
        print('\nSerial communication is unavailable. Pin value cannot be set.\n')
        return 1
    
    # Validate the pin and value
    try:
        pin_index = int(pin_index)
        
        # Handle HIGH/LOW string values
        ...
        # Basic validation - adjust as needed based on Arduino board
        if pin_index < 0 or pin_index > 53:  # Arduino Mega has pins 0-53
            print('\033[31mInvalid pin index. Must be a valid pin number.\033[0m')
            return 1
            
        if value < 0 or value > 255:  # 255 is max for PWM pins
            print('\033[31mInvalid pin value. Must be between 0-255, or HIGH/LOW.\033[0m')
            return 1
    except ValueError:
        print('\033[31mInvalid pin index or value. Pin index must be an integer; value must be HIGH, LOW, or an integer.\033[0m')
        return 1
    
    # Send command format "pin:X:Y" where X is pin index and Y is value
    cmd = 'pin:{}:{}\n'.format(pin_index, value)
    ser.write(cmd.encode('utf-8'))
    t_pin = datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]
    
    # Log the command but don't wait for feedback
    with open(log_path, 'a') as log:
        log.write('Pin command sent at {}: pin {} set to {}\n'.format(t_pin, pin_index, value))
        log.write('\n')
    
    # Use HIGH/LOW in output message if the value is 0 or 1
    if value == 0:
        display_value = "LOW (0)"
    elif value == 1:
        display_value = "HIGH (1)"
    else:
        display_value = value
        
    print(f'Set pin {pin_index} to value {display_value} (no feedback)')
    return 0

def play_bell_sound(log_path, duration=500):
    """
    Play a bell sound as an audible reminder
    
    Parameters:
        log_path (str): Path to log file
        duration (int): Duration of the bell sound in ms (default: 500)
    ...
    t_bell = datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]
    
    try:
        # Different implementations based on the OS
        if current_os == 'Windows':
            # Windows implementation using winsound
            import winsound
            winsound.Beep(880, duration)  # 880 Hz for duration ms
        elif current_os == 'Darwin':  # macOS
            # Use system bell on macOS
            import os
            os.system('afplay /System/Library/Sounds/Glass.aiff')
        else:  # Linux or other
            # Use print with bell character on other platforms
            print('\a')  # ASCII bell character
            time.sleep(duration/1000)  # Convert ms to seconds
            
        # Log the bell event
        with open(log_path, 'a') as log:
            log.write('Bell sound played at {}\n'.format(t_bell))
            log.write('\n')
        ...
        # Log the error
        with open(log_path, 'a') as log:
            log.write('Error playing bell sound at {}: {}\n'.format(t_bell, str(e)))
            log.write('\n')
